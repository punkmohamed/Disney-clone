...
const Recommands = () => {
    ...
    return (
        <div className="py-7 pt-24  ">
            <h3 className=' text-lg sm:text-xl lg:text-3xl '>Recommended for You</h3>
            <div className=" mt-10 grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 justify-center  gap-6">
                {movies && movies?.map((movie) => (
                    <div key={movie.id} className=' recommand relative rounded-xl max-w-[580px] shadow overflow-hidden cursor-pointer  transition-all border border-gray-700 hover:shadow-xl transform hover:scale-110 hover:border-white'>
